...
// C/C++ source code generated on : Thu Oct  8 01:06:21 2020
//
...
void ghost_integrator_step(void)
{
  ...
  if (b_varargout_1 && (ghost_integrator_PrevZCX.Subsystem1_Trig_ZCE !=
                        POS_ZCSIG)) {
    if (ghost_integrator_DW.Subsystem1_RESET_ELAPS_T) {
      elapsedTicks = 0U;
    } else {
      elapsedTicks = ghost_integrator_M->Timing.clockTick0 -
        ghost_integrator_DW.Subsystem1_PREV_T;
    }

    ghost_integrator_DW.Subsystem1_PREV_T =
      ghost_integrator_M->Timing.clockTick0;
    ghost_integrator_DW.Subsystem1_RESET_ELAPS_T = false;

    // DiscreteIntegrator: '<S2>/Discrete-Time Integrator1'
    if (ghost_integrator_DW.DiscreteTimeIntegrator1_SYSTEM_ == 0) {
      // DiscreteIntegrator: '<S2>/Discrete-Time Integrator1'
      ghost_integrator_DW.DiscreteTimeIntegrator1_DSTATE +=
        ghost_integrator_P.DiscreteTimeIntegrator1_gainval * static_cast<real_T>
        (elapsedTicks) * (ghost_integrator_DW.DiscreteTimeIntegrator1_PREV_U +
                          ghost_integrator_B.In1.Twist.Linear.X);
    }

    // End of DiscreteIntegrator: '<S2>/Discrete-Time Integrator1'
    ...
    Pub_ghost_integrator_57.publish(&ghost_integrator_B.BusAssignment);

    // End of Outputs for SubSystem: '<S2>/Publish'

    // Update for DiscreteIntegrator: '<S2>/Discrete-Time Integrator1'
    ghost_integrator_DW.DiscreteTimeIntegrator1_SYSTEM_ = 0U;
    ghost_integrator_DW.DiscreteTimeIntegrator1_PREV_U =
      ghost_integrator_B.In1.Twist.Linear.X;
  }

  ghost_integrator_PrevZCX.Subsystem1_Trig_ZCE = b_varargout_1;
...
{
  {
    ...
    ghost_integrator_PrevZCX.Subsystem1_Trig_ZCE = POS_ZCSIG;
    ...
    ghost_integrator_DW.DiscreteTimeIntegrator1_DSTATE =
      ghost_integrator_P.DiscreteTimeIntegrator1_IC;

    // SystemInitialize for Atomic SubSystem: '<S2>/Publish'
    // Start for MATLABSystem: '<S6>/SinkBlock'
    ghost_integrator_DW.obj_b.matlabCodegenIsDeleted = false;
    ghost_integrator_DW.obj_b.isInitialized = 1;
    Pub_ghost_integrator_57.createPublisher(&b_zeroDelimTopic_tmp[0], 1);
    ghost_integrator_DW.obj_b.isSetupComplete = true;
    ...
    // Enable for Triggered SubSystem: '<Root>/Subsystem1'
    ghost_integrator_DW.Subsystem1_RESET_ELAPS_T = true;

    // Enable for DiscreteIntegrator: '<S2>/Discrete-Time Integrator1'
    ghost_integrator_DW.DiscreteTimeIntegrator1_SYSTEM_ = 1U;
...
void ghost_integrator_terminate(void)
{
  ...
  // Terminate for Triggered SubSystem: '<Root>/Subsystem1'
  // Terminate for Atomic SubSystem: '<S2>/Publish'
  // Terminate for MATLABSystem: '<S6>/SinkBlock'
  if (!ghost_integrator_DW.obj_b.matlabCodegenIsDeleted) {
    ghost_integrator_DW.obj_b.matlabCodegenIsDeleted = true;
  }
